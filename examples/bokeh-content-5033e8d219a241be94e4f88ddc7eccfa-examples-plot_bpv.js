(function() {
  const fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      const element = document.getElementById("91ad9010-378b-4a0a-b581-f0bcb240002d");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '91ad9010-378b-4a0a-b581-f0bcb240002d' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.2.min.js"];
      const css_urls = [];
      
    
      const inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  const docs_json = '{"cd8f8e68-a2ce-4453-a6be-8333a471fee7":{"defs":[],"roots":{"references":[{"attributes":{},"id":"18048","type":"UnionRenderers"},{"attributes":{"toolbars":[{"id":"18009"}],"tools":[{"id":"17999"},{"id":"18000"},{"id":"18001"},{"id":"18002"},{"id":"18003"},{"id":"18004"},{"id":"18005"},{"id":"18006"}]},"id":"18056","type":"ProxyToolbar"},{"attributes":{},"id":"18041","type":"BasicTickFormatter"},{"attributes":{},"id":"18049","type":"Selection"},{"attributes":{},"id":"18005","type":"SaveTool"},{"attributes":{},"id":"17985","type":"DataRange1d"},{"attributes":{},"id":"17992","type":"BasicTicker"},{"attributes":{},"id":"18042","type":"AllLabels"},{"attributes":{},"id":"17983","type":"DataRange1d"},{"attributes":{},"id":"18004","type":"UndoTool"},{"attributes":{"source":{"id":"18025"}},"id":"18030","type":"CDSView"},{"attributes":{"data":{},"selected":{"id":"18051"},"selection_policy":{"id":"18050"}},"id":"18031","type":"ColumnDataSource"},{"attributes":{},"id":"17989","type":"LinearScale"},{"attributes":{},"id":"18002","type":"WheelZoomTool"},{"attributes":{},"id":"18044","type":"BasicTickFormatter"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"18034","type":"Line"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"18008","type":"PolyAnnotation"},{"attributes":{"line_alpha":0.1,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"18027","type":"Line"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"18047"},"selection_policy":{"id":"18046"}},"id":"18018","type":"ColumnDataSource"},{"attributes":{},"id":"18047","type":"Selection"},{"attributes":{},"id":"17987","type":"LinearScale"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"18019","type":"Line"},{"attributes":{},"id":"18046","type":"UnionRenderers"},{"attributes":{"children":[{"id":"18057"},{"id":"18055"}]},"id":"18058","type":"Column"},{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"18037","type":"Title"},{"attributes":{"coordinates":null,"data_source":{"id":"18025"},"glyph":{"id":"18026"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"18028"},"nonselection_glyph":{"id":"18027"},"view":{"id":"18030"}},"id":"18029","type":"GlyphRenderer"},{"attributes":{"coordinates":null,"data_source":{"id":"18031"},"glyph":{"id":"18032"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"18034"},"nonselection_glyph":{"id":"18033"},"view":{"id":"18036"}},"id":"18035","type":"GlyphRenderer"},{"attributes":{"source":{"id":"18031"}},"id":"18036","type":"CDSView"},{"attributes":{},"id":"18045","type":"AllLabels"},{"attributes":{"overlay":{"id":"18008"}},"id":"18003","type":"LassoSelectTool"},{"attributes":{"axis":{"id":"17991"},"coordinates":null,"group":null,"ticker":null},"id":"17994","type":"Grid"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"18007","type":"BoxAnnotation"},{"attributes":{"coordinates":null,"data_source":{"id":"18018"},"glyph":{"id":"18019"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"18021"},"nonselection_glyph":{"id":"18020"},"view":{"id":"18023"}},"id":"18022","type":"GlyphRenderer"},{"attributes":{"data":{"x":[0,1],"y":[1,1]},"selected":{"id":"18049"},"selection_policy":{"id":"18048"}},"id":"18025","type":"ColumnDataSource"},{"attributes":{"children":[[{"id":"17982"},0,0]]},"id":"18055","type":"GridBox"},{"attributes":{"callback":null},"id":"18006","type":"HoverTool"},{"attributes":{"coordinates":null,"formatter":{"id":"18044"},"group":null,"major_label_policy":{"id":"18045"},"ticker":{"id":"17992"}},"id":"17991","type":"LinearAxis"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"18033","type":"Line"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"18020","type":"Line"},{"attributes":{"overlay":{"id":"18007"}},"id":"18001","type":"BoxZoomTool"},{"attributes":{"axis":{"id":"17995"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"17998","type":"Grid"},{"attributes":{"below":[{"id":"17991"}],"center":[{"id":"17994"},{"id":"17998"},{"id":"18024"}],"height":500,"left":[{"id":"17995"}],"output_backend":"webgl","renderers":[{"id":"18022"},{"id":"18029"},{"id":"18035"}],"title":{"id":"18037"},"toolbar":{"id":"18009"},"toolbar_location":null,"width":500,"x_range":{"id":"17983"},"x_scale":{"id":"17987"},"y_range":{"id":"17985"},"y_scale":{"id":"17989"}},"id":"17982","subtype":"Figure","type":"Plot"},{"attributes":{"bottom":1.459677659268402,"coordinates":null,"fill_alpha":0.1,"fill_color":"#2a2eec","group":null,"top":0.6850827603275131},"id":"18024","type":"BoxAnnotation"},{"attributes":{"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"18032","type":"Line"},{"attributes":{"toolbar":{"id":"18056"},"toolbar_location":"above"},"id":"18057","type":"ToolbarBox"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"18021","type":"Line"},{"attributes":{"source":{"id":"18018"}},"id":"18023","type":"CDSView"},{"attributes":{},"id":"17999","type":"ResetTool"},{"attributes":{"line_alpha":0.2,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"18028","type":"Line"},{"attributes":{},"id":"17996","type":"BasicTicker"},{"attributes":{"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"18026","type":"Line"},{"attributes":{"coordinates":null,"formatter":{"id":"18041"},"group":null,"major_label_policy":{"id":"18042"},"ticker":{"id":"17996"}},"id":"17995","type":"LinearAxis"},{"attributes":{},"id":"18000","type":"PanTool"},{"attributes":{},"id":"18051","type":"Selection"},{"attributes":{},"id":"18050","type":"UnionRenderers"},{"attributes":{"tools":[{"id":"17999"},{"id":"18000"},{"id":"18001"},{"id":"18002"},{"id":"18003"},{"id":"18004"},{"id":"18005"},{"id":"18006"}]},"id":"18009","type":"Toolbar"}],"root_ids":["18058"]},"title":"Bokeh Application","version":"2.4.2"}}';
                  const render_items = [{"docid":"cd8f8e68-a2ce-4453-a6be-8333a471fee7","root_ids":["18058"],"roots":{"18058":"91ad9010-378b-4a0a-b581-f0bcb240002d"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();